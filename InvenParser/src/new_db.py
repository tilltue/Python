...
def findOriginalCode(eng_name,cardJson):
	root = langRoots['enUS']
	for entity in root.findall(".//Entity") :
		...
		if entity != None :
			...
			cardJson['cost'] = getTagValue(entity,48)
			cardJson['attack'] = getTagValue(entity,47)
			health = getTagValue(entity,45)
			if len(health) == 0 :
				health = getTagValue(entity,187)
			cardJson['health'] = health
			cardJson['rarity'] = getTagValue(entity,203)
			cardJson['cardClass'] = getTagValue(entity,199)
			cardJson['artist'] = getTagText(entity,342)
			getRace(entity,cardJson)
			langJson = []
...
def cardEngParser(result_cards,urlString):
	...
	for card in cards :
		...
		"""
		if saveCard == True :
			#findOriginalCode(cardName,result_card)
